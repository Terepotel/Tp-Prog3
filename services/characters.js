...
import { getFileContent, writeFileContent } from '../fileManagement/filesManagement.js'
...
export const reduceCharactersIntoNames = async (source, output) => {
  try {
    const characters = JSON.parse(await getFileContent(source || './data/characters.json'))
